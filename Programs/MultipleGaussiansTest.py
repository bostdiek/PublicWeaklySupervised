...
#     Make the test data set: Stays the same for all cases
    ...
    while i <= samples:
        ...
        Set_70_30 = MakeMultiGSamples(set0nums, set1nums, 0.7, 0.7 + float(err)/100, 200000)
        ...
        WeaklyModel.add(Dense(30, input_dim=3, init='normal',activation='sigmoid'))
        WeaklyModel.add(Dense(1, init='normal', activation='sigmoid'))
        WeaklyModel.compile(loss=WeakSupervision,optimizer=Adam(lr=0.00009, clipnorm=1.))
        
        #Train
        checkpointer = ModelCheckpoint('models/MuliG_weights_'+str(err)+'_'+str(i)+'.h5', 
                          monitor='val_loss',
                          save_best_only=True)
        ...
        WeaklyModel.load_weights('models/MuliG_weights_'+str(err)+'_'+str(i)+'.h5')                  
        ...
        if auc_score != 0:
            ...
            with open('data/MuliG_weights_fpr_' + str(err) + '_'+str(i) + '.dat','w') as ffpr:
                for l in fpr:
                    ffpr.write(str(l))
                    ffpr.write('\n')
            with open('data/MuliG_weights_tpr_'+ str(err) + '_' + str(i) + '.dat','w') as tfpr:
                for l in tpr:
                    tfpr.write(str(l))
                    tfpr.write('\n')
            df=pd.DataFrame(outarray[1:],columns=outarray[0])
            df.to_csv('data/MuliG_Error_InFraction_' + str(err) + '.dat',index=False)
...
if __name__ == "__main__":
    ...
    if len(argv) != 2:
        ...
        print "...> python MultipleGaussiansTest.py ErrorSize SampleSize"
        print "where ErrorSize and SampleSize are integer values\n"
