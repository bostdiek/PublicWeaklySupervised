...
def MakeErrorTrials(err, samples=100):
    ...
    the ROC curve and the area under the curve are saved. The trained model
    weights are also saved for later use. There is no output of the function,
    but results are saved to disk.
...
#     Make the test data set: Stays the same for all cases
    ...
    Set_70_30 = MakeMultiGSamples(set0nums,
                                  set1nums,
                                  0.7,
                                  float(err) / 100,
                                  200000
    ...
    x_train, x_valid, y_train, y_valid =\
        train_test_split(np.vstack([XTrain1, XTrain2]),
    ...
    while i <= samples:
        ...
        WeaklyModel.add(Dense(30, activation="sigmoid",
                              kernel_initializer="normal", input_dim=3))
        WeaklyModel.add(Dense(1, activation="sigmoid",
                              kernel_initializer="normal"))
        WeaklyModel.compile(loss=WeakSupervision,
                            optimizer=Adam(lr=0.00009, clipnorm=1.))

        # Train
        checkpointer = ModelCheckpoint('../Data/KerasModelWeights/MuliG_weights_' +
                                       str(err) + '_' + str(i) + '.h5',
                                       monitor='val_loss',
                                       save_best_only=True)
        ...
        WeaklyModel.load_weights('../Data/KerasModelWeights/MuliG_weights_' +
                                 str(err) + '_' + str(i) + '.h5')
        ...
        if auc_score != 0:
            if auc_score < 0.5:  # got flipped
                ...
                tpr, fpr, thresh = metrics.roc_curve(YTest[:], preds[:])
                auc_score = metrics.auc(fpr, tpr)
            outarray.append([err, i, (float(err) / 100.) / 0.7,
                             YTrainPercentage2[0], auc_score])
            print "AUC:{0}".format(auc_score)
            with open('../Data/TrueFalsePositiveRates/MuliG_weights_fpr_' +
                      str(err) + '_' + str(i) + '.dat', 'w') as ffpr:
                for l in fpr:
                    ffpr.write(str(l))
                    ffpr.write('\n')
            with open('../Data/TrueFalsePositiveRates/MuliG_weights_tpr_' +
                      str(err) + '_' + str(i) + '.dat', 'w') as tfpr:
                for l in tpr:
                    tfpr.write(str(l))
                    tfpr.write('\n')
            df = pd.DataFrame(outarray[1:], columns=outarray[0])
            df.to_csv('../Data/TrueFalsePositiveRates/MuliG_Error_InFraction_' +
                      str(err) + '.dat', index=False)
